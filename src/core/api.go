...
// Address gets the IPv6 address of the Uqda node. This is always a /128
...
// Subnet gets the routed IPv6 subnet of the Uqda node. This is always a
...
// SetLogger sets the output logger of the Uqda node after startup. This
...
// AddPrivateNetworkAllowedKey adds a member key from PrivateNetworks config (runtime updates).
func (c *Core) AddPrivateNetworkAllowedKey(pk ed25519.PublicKey) {
	phony.Block(c, func() {
		var k [32]byte
		copy(k[:], pk)
		c.config._privateNetworkAllowedKeys[k] = struct{}{}
	})
}

// ReplacePrivateNetworkAllowedKeys replaces the private-network allowlist (e.g. after leaveNetwork).
func (c *Core) ReplacePrivateNetworkAllowedKeys(pks []ed25519.PublicKey) {
	phony.Block(c, func() {
		m := make(map[[32]byte]struct{}, len(pks))
		for _, pk := range pks {
			var k [32]byte
			copy(k[:], pk)
			m[k] = struct{}{}
		}
		c.config._privateNetworkAllowedKeys = m
	})
}
