	...
	phony.Block(l, func() {
		...
		go func() {
			...
			for {
				...
				// Clear the error state (matches inbound accept path).
				var doRet bool
				phony.Block(l, func() {
					if state._conn != nil {
...
func (l *links) handler(linkType linkType, options linkOptions, conn net.Conn, success func(), local bool) error {
	...
	if !local {
		var globalAllowed, privateAllowed map[[32]byte]struct{}
		phony.Block(l.core, func() {
			globalAllowed = l.core.config._allowedPublicKeys
			privateAllowed = l.core.config._privateNetworkAllowedKeys
		})
		// Allow all only when both global and private-network allowlists are empty.
		isallowed := len(globalAllowed) == 0 && len(privateAllowed) == 0
		if !isallowed {
			var k [32]byte
			copy(k[:], meta.publicKey)
			if _, ok := globalAllowed[k]; ok {
				isallowed = true
			}
			if !isallowed {
				if _, ok := privateAllowed[k]; ok {
					isallowed = true
				}
			}
		}
		if linkType == linkTypeIncoming && !isallowed {
			return fmt.Errorf("node public key %q is not in AllowedPublicKeys or private network allowlist", hex.EncodeToString(meta.publicKey))
		}
	}
...
func (l *links) findSuitableIP(url *url.URL, fn func(hostname string, ip net.IP, port int) (net.Conn, error)) (net.Conn, error) {
	...
	network := l.core.config.peerDialNetwork
	if network == "" {
		network = "ip"
	}
	// Use DNS cache to reduce lookup latency
	resp, err := lookupIPWithCache(host, network)
	if err != nil {
		return nil, err
	}
	var _ips [64]net.IP
	...
	if l.core.config.preferIPv4 && len(ips) > 1 {
