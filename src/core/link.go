...
// IsValidListenScheme reports whether scheme is a URI scheme recognised
...
// IsValidPeerScheme reports whether scheme is a URI scheme recognised for
// outbound peer URIs (see links.dialerFor's switch statement, which this
// must be kept in sync with). See IsValidListenScheme for why this is
// exported.
func IsValidPeerScheme(scheme string) bool {
	switch strings.ToLower(scheme) {
	case "tcp", "tls", "socks", "sockstls", "unix", "quic", "ws", "wss":
		return true
	default:
		return false
	}
