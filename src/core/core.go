...
type Core struct {
	...
	*iwe.PacketConn
	...
	config       struct {
		...
		peerDialNetwork    string                     // immutable: "", "ip", "ip4", "ip6" for DNS when dialling peers
		preferIPv4         bool                       // immutable: try IPv4 before IPv6 when both exist
		nodeinfo           NodeInfo                   // immutable after startup
		nodeinfoPrivacy    NodeInfoPrivacy            // immutable after startup
		_allowedPublicKeys          map[[32]byte]struct{} // global AllowedPublicKeys from config
		_privateNetworkAllowedKeys map[[32]byte]struct{} // union of PrivateNetworks[].allowed_keys
	}
...
func New(cert *tls.Certificate, logger Logger, opts ...SetupOption) (*Core, error) {
	...
	c.config._privateNetworkAllowedKeys = map[[32]byte]struct{}{}
