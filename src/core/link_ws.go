...
func (s *wsServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "ygg-ws", "uqda-ws":
		// Accepted — either the legacy Yggdrasil network subprotocol
		// or the native UQDA subprotocol is allowed, so we can
		// interoperate with both networks.
	default:
		c.Close(websocket.StatusPolicyViolation, "client must speak the ygg-ws or uqda-ws subprotocol")
		return
	}
