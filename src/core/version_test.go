...
// FuzzVersionMetadataDecode fuzzes the handshake parser that runs on every
// inbound and outbound peering attempt (src/core/link.go's l.handler calls
// meta.decode directly on attacker-controlled bytes before any
// authentication has taken place). The only property under test is that
// decode never panics and never reads past what was written to it -
// rejecting malformed input with an error is the correct, expected outcome
// for arbitrary fuzzer-generated bytes.
func FuzzVersionMetadataDecode(f *testing.F) {
	// Seed with the "meta" preamble so the fuzzer starts inside the format
	// rather than needing to discover the 4-byte magic from scratch.
	pk, sk, err := ed25519.GenerateKey(nil)
	if err != nil {
		f.Fatal(err)
	}
	...
	f.Add(malformedVersionHandshake(f, metaVersionMajor, []byte{1}, nil))
	f.Add([]byte("meta"))
	f.Add([]byte{})
...
func malformedVersionHandshake(t testing.TB, op uint16, field []byte, password []byte) []byte {
	t.Helper()
