...
type NodeConfig struct {
	...
	PeerDialNetwork     string                     `json:",omitempty" comment:"DNS resolution for outbound peer hostnames: empty or \"ip\" (default) uses both IPv4 and IPv6; \"ip4\" only A records; \"ip6\" only AAAA records."`
	PreferIPv4          bool                       `json:",omitempty" comment:"If true, when both address families are available from DNS, IPv4 is tried before IPv6."`
	...
	AdminAuth           string                     `json:",omitempty" comment:"If non-empty, every admin API request must include the same value in\nthe JSON \"auth\" field (see Uqdactl -auth or UQDA_ADMIN_AUTH). Leave\nempty to keep the previous behaviour (no token check)."`
	MetricsListen       string                     `json:",omitempty" comment:"If non-empty, serve Prometheus /metrics on this address (e.g. 127.0.0.1:9090).\nBind to loopback unless you trust the network. Empty disables metrics HTTP."`
	UIListen            string                     `json:",omitempty" comment:"If non-empty, serve the Web UI dashboard on this HTTP address (e.g. localhost:9002).\nProxies the Admin API at POST /api/<command>. Bind to loopback in production.\nEmpty disables the Web UI."`
	MulticastInterfaces []MulticastInterfaceConfig `comment:"Configuration for which interfaces multicast peer discovery should be\nenabled on. Regex is a regular expression which is matched against an\ninterface name, and interfaces use the first configuration that they\nmatch against. Beacon controls whether or not your node advertises its\npresence to others, whereas Listen controls whether or not your node\nlistens out for and tries to connect to other advertising nodes. See\nhttps://Uqda-network.github.io/configurationref.html#multicastinterfaces\nfor more supported options."`
	...
	PrivateNetworks     []PrivateNetwork           `json:",omitempty" comment:"Private invite groups: multicast password, member keys, and bootstrap peers (see uqdactl network)."`
