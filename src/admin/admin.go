...
// adminMaxRequestBodyBytes caps each JSON admin request to limit memory use from a single client.
const adminMaxRequestBodyBytes = 1 << 20 // 1 MiB
...
type AdminSocket struct {
	...
	authToken      string // if non-empty, JSON field "auth" must match (see AuthToken option)
	config         struct {
	...
	if err == nil {
		...
		case "unix":
			if _, err := os.Stat(u.Path); err == nil {
				...
				conn, dialErr := net.DialTimeout("unix", u.Path, time.Second*2)
				if dialErr == nil {
					_ = conn.Close()
...
func (a *AdminSocket) handleRequest(conn net.Conn) {
	...
	for {
		...
		req.Arguments = []byte("{}")
		if err := func() error {
			limited := io.LimitReader(conn, adminMaxRequestBodyBytes)
			...
			if err := a.requireAuth(&req); err != nil {
				return err
			}
...
func (a *AdminSocket) requireAuth(req *AdminSocketRequest) error {
	if a.authToken == "" {
		return nil
	}
	if subtle.ConstantTimeCompare([]byte(req.Auth), []byte(a.authToken)) != 1 {
		return fmt.Errorf("admin authentication required or invalid")
	}
	return nil
}
