...
// SECURITY NOTICE: The admin socket currently has no authentication.
// Do NOT expose the admin socket to untrusted networks.
// Default path: unix:///var/run/uqda.sock (Unix socket, root-only by default)
// If using TCP (AdminListen: tcp://...), bind to localhost only
