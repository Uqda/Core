/*
This file generates host_vars and vault key material for use with Ansible
and Uqda. See https://github.com/Uqda/Core
*/
package main
...
func main() {
	...
	for i := 1; i <= *numHosts; i++ {
		...
		defer file.Close()
		file.WriteString(fmt.Sprintf("uqda_public_key: %v\n", hex.EncodeToString(keys[i].pub)))
		file.WriteString("uqda_private_key: \"{{ vault_uqda_private_key }}\"\n")
		...
		defer file.Close()
		file.WriteString(fmt.Sprintf("vault_uqda_private_key: %v\n", hex.EncodeToString(keys[i].priv)))
		bar.Increment()
	}
	bar.Finish()
}
