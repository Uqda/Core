...
Last updated: after the product rename to Uqda (module path, binaries, config/service paths, all packaging, README/docs/CI).
...
| 1 | Product rename execution (module path, binaries, config paths, packaging, docs) | **Tested** | `go.mod` -> `github.com/Uqda/Core`, `cmd/uqda`/`cmd/uqdactl`, config/service paths under `/etc/uqda` etc., all `contrib/` packaging, README + [README.ar.md](../README.ar.md), CI workflow names. Full build/vet/test green throughout, done in ~10 incremental commits (module+cmd -> cli/build scripts -> config paths/doc comments -> packaging -> CI -> docs). Not done: `src/` internal package boundaries (that's Phase 3, a separate and much larger effort - see that row below), router/OpenWrt packaging (dropped, see Phase 21), and the real pinned-upstream interop fixture (Phase 16) |
...
| 21 | Platform support validation | **Not started** | Router/OpenWrt/EdgeRouter/VyOS packaging dropped from CI (it depended on an external third-party repo built for the original yggdrasil/yggdrasilctl binary names, incompatible with the rename) rather than left broken - a real gap, no Uqda-side replacement exists yet |
...
| 24 | Full documentation set | **In progress** | ARCHITECTURE.md, NAMING.md, RESTRUCTURING.md, BASELINE.md, [SECURITY.md](../SECURITY.md), [docs/THREAT_MODEL.md](THREAT_MODEL.md), [NOTICE.md](../NOTICE.md), [README.md](../README.md) (rewritten with the required "independently maintained, hardened and modernized implementation compatible with the existing Yggdrasil network" framing and the عُقدة name explanation), [README.ar.md](../README.ar.md) all exist. Still pending: a dedicated upstream-comparison doc distinguishing inherited-vs-Uqda features point by point (the naming/status docs cover this narratively but not as a single table) |
