...
شبكة Uqda هي بروتوكول توجيه لامركزي مصمم لبناء شبكات mesh متعددة القفزات مرنة وذاتية التنظيم مع تشفير من طرف إلى طرف، وإعدادات دنيا، واستقلالية عن البنية التحتية المركزية.

المبنية على بدائيات تشفير مثبتة وخوارزميات توجيه مبتكرة، تمكن Uqda من الاتصال المباشر بين الأقران عبر طوبولوجيات شبكية ديناميكية مع الحفاظ على عنونة مستقرة ومستقلة عن الموقع من خلال IPv6.

تقدم هذه الوثيقة البنية التقنية، ومبادئ التصميم، والتحسينات العملية التي تميز Uqda كحل عملي لتحديات الشبكات اللامركزية الحديثة.
...
تعالج Uqda هذه المتطلبات من خلال نهج مختلف جوهرياً لتوجيه الشبكة وتنظيمها.

### 1.2 فلسفة التصميم

Uqda مبنية على ثلاثة مبادئ أساسية:
...
Uqda Core هو **عمل مشتق** من بروتوكول شبكة Yggdrasil (v0.5)، يُحافظ عليه كمشروع مستقل مع التركيز على:

- **تحسينات الأداء** للنشر في الإنتاج
- **تحسينات تشغيلية** بناءً على الاستخدام العملي
- **أدوات محسّنة** للمراقبة والتشخيص

**التوافق مع البروتوكول**: عقد Uqda متوافقة مع البروتوكول مع عقد Yggdrasil v0.5 ويمكنها الاتصال معها بسلاسة.

**الحوكمة**: تعمل Uqda كمشروع مستقل بدورة إصدار خاصة بها، وأولويات، وخارطة طريق تطوير.
...
تعمل Uqda كشبكة **overlay** تعمل فوق البنية التحتية IP الموجودة، رغم أن تصميم البروتوكول لا يتطلب بشكل أساسي تشغيل overlay.

كل عقدة شبكة تتكون من:
...
تمتلك كل عقدة **زوج مفتاح عام/خاص** (Ed25519):

- **المفتاح العام** يخدم كهوية العقدة الحقيقية
...
معظم التطبيقات القادرة على IPv6 تعمل على Uqda **بدون تعديل**، حيث تقدم الشبكة عناوين IPv6 قياسية لنظام التشغيل المضيف.
...
- **أعداء الشبكة**: قد تكون أي عقدة أو رابط اتصال خاضعاً لسيطرة مهاجم
...
### 6.3 ما ليست عليه Uqda

**Uqda ليست شبكة إخفاء هوية.** لا توفر:
...
**الحالة الحالية**: Uqda **لم تخضع لتدقيق أمني مستقل**.
...
تدمج Uqda تحسينات عملية بناءً على خبرة النشر في العالم الحقيقي:
...
| المقياس | الأساسي (Yggdrasil 0.5.12) | Uqda 0.1.1 | التحسين |
|--------|------------------------------|------------|-------------|
...
- قائمة انتظار عادلة للحزم تمنع تشبع عقدة واحدة

### 8.2 معالجة MTU

تستخدم Uqda **قيم MTU عالية** (حتى 65535) على واجهات TUN:
...
كشبكة **عامة**، عقد Uqda قابلة للوصول عالمياً افتراضياً.
...
- حظر جميع حركة مرور IPv6 الواردة على واجهة Uqda افتراضياً
...
رغم أن Uqda يمكن استخدامها للشبكات الخاصة، إلا أنها **غير مصممة كـ VPN تقليدي**:
...
| الميزة | Uqda | Yggdrasil | Tailscale | Nebula | I2P/Tor |
|---------|------|-----------|-----------|--------|---------|
| لامركزي | ✅ | ✅ | ❌ (منسق) | ❌ (منارة) | ✅ |
...
تمثل Uqda **تطوراً يركز على الإنتاج** لمبادئ التوجيه اللامركزي المثبتة، محسّنة لـ:
...
Uqda **لا** تحل جميع مشاكل الشبكات. إنها:
...
تعيد Uqda السيطرة على بنية الشبكة التحتية لمستخدميها.  
...
Uqda Core مبنية على بروتوكول Yggdrasil Network، مع تحسينات تنفيذ كبيرة وتحسينات تشغيلية. راجع [ATTRIBUTION.md](../ATTRIBUTION.md) للاعتمادات الكاملة.
