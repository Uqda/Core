...
**Prerequisites:**
- Go 1.25.8 or later (see `go.mod` / CI)
- Git
...
Use **golangci-lint v2** (see `.golangci.yml`). Install a binary matching CI, e.g.:
...
Same checks as CI: `go mod verify`, `go vet`, `govulncheck`, build all packages, tests.

- **Linux / macOS:** `chmod +x scripts/verify.sh && ./scripts/verify.sh`
- **Windows (PowerShell):** `.\scripts\verify.ps1`

Optional: `./scripts/verify.sh --race` or `.\scripts\verify.ps1 -Race` for `-race` (slower). Use `--skip-vuln` / `-SkipVuln` if offline.
