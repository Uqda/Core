...
### Release 1.0.0 & project vision

The repository baseline is **[v1.0.0](CHANGELOG.md)**. For the **full project narrative** (Arabic + English) — goals, scope, and values — see **[PROJECT_VISION.md](PROJECT_VISION.md)**.

**رؤية مختصرة:** **عُقدة** شبكة تراكب IPv6 مشفّرة من طرف إلى طرف؛ عنوانك الثابت مشتق من مفتاحك، والنسيج يُعيد اكتشاف المسارات دون سجلّ مركزي. التفاصيل الكاملة في `PROJECT_VISION.md`.
...
**From Source**
```bash
# Prerequisites: Go 1.25.8+ (see `go.mod` toolchain)
...
### Prerequisites
- Go 1.25.8 or later (recommended: match `toolchain` in `go.mod`)
- Git
...
- **[Project vision](PROJECT_VISION.md)** — Full vision (Arabic + English)
...
Set `UIListen` in your config (for example `localhost:9002`), restart the node, then open `http://localhost:9002` in a browser. The daemon serves a single-page dashboard that proxies the Admin API over HTTP (use loopback or a trusted reverse proxy; set `AdminAuth` for token protection).
