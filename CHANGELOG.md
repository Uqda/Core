...
* Protocol metadata now includes a cryptographically random per-connection nonce.
* Signatures now authenticate every metadata field, including protocol versions and link priority.
* A second signed confirmation flight binds both peers' identities, nonces and complete hello messages, preventing metadata replay and downgrade attacks.
...
* The link protocol minor version is now 0.6. Both peers must upgrade before they can connect.
