...
func run() int {
	...
	if cmdLineEnv.adminAuth == "" {
		cmdLineEnv.adminAuth = os.Getenv("UQDA_ADMIN_AUTH")
	}
	cmdLineEnv.setEndpoint(logger)
	mergeAdminAuthFromDefaultConfig(&cmdLineEnv, logger)
	...
	if len(cmdLineEnv.args) >= 1 && strings.EqualFold(cmdLineEnv.args[0], "network") {
		return runNetworkCLI(conn, &cmdLineEnv)
	}

	decoder := json.NewDecoder(conn)
	encoder := json.NewEncoder(conn)
	send := &admin.AdminSocketRequest{Auth: cmdLineEnv.adminAuth}
