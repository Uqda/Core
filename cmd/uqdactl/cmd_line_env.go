...
func (cmdLineEnv *CmdLineEnv) parseFlagsAndArgs() {
	flag.Usage = func() {
		_, _ = fmt.Fprintf(flag.CommandLine.Output(), "Usage: %s [options] command [key=value] [key=value] ...\n\n", os.Args[0])
		fmt.Println("Options:")
		flag.PrintDefaults()
		fmt.Println()
		...
		fmt.Println("  - ", os.Args[0], `network create -name home -peer tls://1.2.3.4:12345`)
		fmt.Println("  - ", os.Args[0], `network join uqda-invite-v1-...`)
	}

	server := flag.String("endpoint", cmdLineEnv.endpoint, "Admin socket endpoint")
	auth := flag.String("auth", "", "Admin API shared secret if the node uses AdminAuth (or set UQDA_ADMIN_AUTH)")
...
// mergeAdminAuthFromDefaultConfig sets adminAuth from the platform default config file when still unset.
func mergeAdminAuthFromDefaultConfig(cmdLineEnv *CmdLineEnv, logger *log.Logger) {
	if cmdLineEnv.adminAuth != "" {
		return
	}
	cfg, err := os.ReadFile(config.GetDefaults().DefaultConfigFile)
	if err != nil {
		return
	}
	...
	if a, ok := dat["AdminAuth"].(string); ok && a != "" {
		cmdLineEnv.adminAuth = a
		logger.Println("Using AdminAuth from default config file")
	}
}
