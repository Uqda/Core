...
func main() {
	...
	case "syslog":
		if syslogger, err := gsyslog.NewLogger(gsyslog.LOG_NOTICE, "DAEMON", version.Name()); err == nil {
	...
	case *useconf:
		if _, err := cfg.ReadFrom(os.Stdin); err != nil {
			logger.Fatalf("read config from stdin: %v", err)
		}

	case *useconffile != "":
		f, err := os.Open(*useconffile)
		if err != nil {
			logger.Fatalf("open config file %q: %v", *useconffile, err)
		}
		if _, err := cfg.ReadFrom(f); err != nil {
			_ = f.Close()
			logger.Fatalf("parse config file %q: %v", *useconffile, err)
		}
		_ = f.Close()
