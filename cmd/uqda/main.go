...
func main() {
	...
	// Set up the Uqda node itself.
	{
		...
		options := []core.SetupOption{
			...
			// Reject overlay mesh range (200::/7) as an underlay hop target; IPv4 and public IPv6 are allowed.
			core.PeerFilter(func(ip net.IP) bool {
				return !iprange.Contains(ip)
			}),
		}
		if nw := strings.TrimSpace(cfg.PeerDialNetwork); nw != "" {
			options = append(options, core.PeerDialNetwork(nw))
		}
		if cfg.PreferIPv4 {
			options = append(options, core.PreferIPv4(true))
		}
		...
		pnKeys, err := config.PrivateNetworkAllowedKeyHex(cfg)
		if err != nil {
			panic(err)
		}
		for _, k := range pnKeys {
			options = append(options, core.PrivateNetworkAllowedKey(k))
		}
		for _, pn := range cfg.PrivateNetworks {
			for _, p := range pn.Peers {
				options = append(options, core.Peer{URI: p})
			}
		}
	...
	{
		warnIfAdminListenInsecureWithoutAuth(cfg.AdminListen, cfg.AdminAuth, logger)
		options := []admin.SetupOption{
			admin.ListenAddress(cfg.AdminListen),
			admin.AuthToken(cfg.AdminAuth),
		}
		if cfg.LogLookups {
	...
	if addr := strings.TrimSpace(cfg.MetricsListen); addr != "" {
		startMetricsServer(addr, logger)
	}

	if addr := strings.TrimSpace(cfg.UIListen); addr != "" {
		warnIfUIListenNotLoopback(addr, logger)
		startUIServer(addr, cfg.AdminListen, cfg.MetricsListen, cfg.AdminAuth, logger)
	}
